...
export async function getLogs(query: Record<string, unknown>): Promise<PaginatedLogs> {
  ...
  if (query.limit === 'all' || query.limit === '0' || query.limit === 0) {
    limit = 50000;
  } else {
    const parsedLimit = parseInt(String(query.limit ?? DEFAULT_LIMIT), 10);
    limit = isNaN(parsedLimit) || parsedLimit <= 0 ? DEFAULT_LIMIT : parsedLimit;
  }
