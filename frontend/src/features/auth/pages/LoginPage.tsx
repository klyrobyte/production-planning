...
export default function LoginPage() {
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col items-center justify-center bg-slate-50 px-6 py-12 select-none overflow-y-auto">
      ...
      <div className="w-full max-w-6xl flex flex-col items-center justify-between gap-4 md:flex-row md:absolute md:top-8 md:px-12 mb-8 md:mb-0">
        <div className="flex items-center gap-3">
          <div className="flex h-12 w-16 items-center justify-center rounded-xl bg-white p-1.5 shadow-sm border border-slate-100 shrink-0">
            <img src={systemLogo || '/logo.png'} alt="SC Logo" className="max-h-full max-w-full object-contain" />
          </div>
          <div className="text-left">
            <h1 className="text-sm font-black uppercase tracking-tight text-slate-800 leading-none">{systemTitle}</h1>
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1">Integrated Production Planning System</p>
          </div>
        </div>

        {isAuthenticated && user && (
          <div className="flex items-center gap-4 rounded-2xl bg-white p-2.5 pl-4 pr-3.5 shadow-sm border border-slate-100">
            <p className="text-xs font-bold text-slate-500">
              Workstation Active: <span className="font-extrabold text-slate-800 uppercase">{user.name}</span>
            </p>
            <button
              onClick={() => logoutDevice()}
              className="flex items-center gap-1.5 rounded-xl bg-slate-50 px-3 py-1.5 text-[10px] font-black uppercase tracking-wider text-slate-600 transition hover:bg-slate-100 hover:text-slate-900 cursor-pointer"
            >
      ...
      <div className="w-full max-w-md text-center z-10 animate-in fade-in slide-in-from-bottom-4 duration-300">
        {!isAuthenticated ? (
          ...
          <div className="rounded-3xl bg-white p-8 shadow-xl border border-slate-100">
            ...
            <h2 className="mt-4 text-xl font-black uppercase tracking-wider text-slate-800">Device Authorization</h2>
            <p className="mt-1 text-xs font-bold text-slate-400 uppercase tracking-wide">Log in to unlock this workstation</p>
            <div className="mt-8">
              <DeviceLoginForm />
            </div>
          </div>
          ...
          <div className="rounded-3xl bg-white p-8 shadow-xl border border-slate-100">
            ...
            <h2 className="mt-4 text-xl font-black uppercase tracking-wider text-slate-800">Member Machine Login</h2>
            <p className="mt-1 text-xs font-bold text-slate-400 uppercase tracking-wide">Pilih stasiun mesin dan masukkan PIN</p>
            <div className="mt-8">
