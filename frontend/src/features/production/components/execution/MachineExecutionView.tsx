...
export function MachineExecutionView({
  machine,
  factory,
  machineKey: propsMachineKey,
  selectedDate,
}: MachineExecutionViewProps) {
  ...
  const connectBluetoothPrinter = async () => {
    ...
    try {
      ...
      if (!result) {
        throw new Error(
          'Tidak ditemukan characteristic yang bisa di-write pada printer ini. Pastikan printer dalam mode aktif.'
        );
      }
      ...
      useBtPrinterStore.setState({
        connectionError: err.message || 'Failed to connect. Ensure printer is paired.',
        connectionStatus: 'error',
      });
    }
  };
  ...
  const handlePrintSuccess = (printedQty: number) => {
    ...
    incrementJobProgress(machineKey, activeJob.id, printedQty, selectedDate, partsData, {
      type: 'progress',
      note: getLogNote(`Print label ${activeJob.model}: +${printedQty} pcs`),
    });
  };
  ...
  const printLockStatus = useMemo(() => {
    ...
    if (ngState.isNg && ngState.start) {
      ...
      if (startParts.length === 2) {
        ...
        ongoingDowntimeSecs = Math.max(ongoingDowntimeSecs, ngDowntimeSecs);
      }
    }
  ...
  const handleCompleteFinalDandori = (jobId: string) => {
    updateJobStatus(machineKey, jobId, 'complete-final-dandori', selectedDate, partsData, {
      type: 'success',
      note: getLogNote(`Dandori Akhir Shift selesai untuk mesin ${mName}.`),
    });
  };
  ...
  const handleSignOffSubmit = async () => {
    ...
    if (!allChecked) {
      setPinError('Centang semua checklist dulu.');
      return;
    }
    if (!leaderPin || leaderPin.length < 4) {
      setPinError('PIN Leader wajib diisi (min 4 digit).');
      return;
    }
    setPinLoading(true);
    setPinError('');
  ...
  const handleReportAbnormality = () => {
    if (!abnormality.isAbnormal) {
      ...
      setIsReportingAbnormal(true);
      setMachineAbnormal(machineKey, true, 'Dalam Investigasi...', now, selectedDate, {
        type: 'abnormal',
        note: getLogNote(`Abnormal STARTED — jenis sedang diinput`),
      });
    }
  };

  const handleSaveAbnormalityRecord = () => {
    setMachineAbnormal(machineKey, true, selectedAbnType, abnormalStartTime, selectedDate, {
      type: 'abnormal',
      note: getLogNote(`Abnormal STARTED: ${selectedAbnType}`),
    });
    setIsReportingAbnormal(false);
  };

  const handleCancelAbnormality = () => {
    setMachineAbnormal(machineKey, false, '', '', selectedDate, {
      type: 'success',
      note: getLogNote(`Abnormal DIBATALKAN`),
    });
    setIsReportingAbnormal(false);
  };
    ...
    if (abnormality.isAbnormal) {
      ...
      setMachineAbnormal(
        ...
        selectedDate,
        {
          type: 'abnormal',
          note: getLogNote(
            `Abnormal RESOLVED: ${abnormality.type}. Downtime: ${dMins} min. Start: ${abnormality.start} | Stop: ${nowStr}`
          ),
          timeStr: nowStr,
        },
        dMins,
  ...
  const handleReportNg = () => {
    if (!ngState.isNg) {
      ...
      setNgStartTime(now);
      setIsReportingNg(true);
      setMachineNg(machineKey, true, 'Dalam Investigasi...', now, selectedDate, {
        type: 'ng',
        note: getLogNote(`NG STARTED — jenis sedang diinput`),
      });
    }
  };

  const handleSaveNgRecord = () => {
    setMachineNg(machineKey, true, selectedNgType, ngStartTime, selectedDate, {
      type: 'ng',
      note: getLogNote(`NG STARTED: ${selectedNgType}`),
    });
    setIsReportingNg(false);
  };
  ...
  const handleResolveNg = () => {
    if (ngState.isNg) {
      ...
      setMachineNg(machineKey, false, '', '', selectedDate, {
        type: 'ng',
        note: getLogNote(`NG RESOLVED: ${ngState.type}. Start: ${ngState.start} | Stop: ${now}`),
        timeStr: now,
      });
    }
  };
    ...
    <div className="flex-1 overflow-y-auto bg-slate-100 dark:bg-slate-900 p-4 sm:p-6 space-y-5">
        ...
        <div className="flex items-center gap-3 px-4 py-3 bg-blue-50 border border-blue-200 dark:bg-blue-900/30 dark:border-blue-800 rounded-xl text-blue-800 dark:text-blue-300 shadow-sm animate-in fade-in">
          ...
          <div className="flex-1">
            ...
            <p className="text-[10px] mt-0.5 font-semibold">
              Anda sedang melihat jadwal untuk tanggal {selectedDate}. Eksekusi job hanya diizinkan untuk jadwal hari
              ini, kecuali Anda adalah Admin.
            </p>
          </div>
        </div>
      )}
      ...
      {abnormality.isAbnormal && (
        <div className="flex items-center gap-3 px-4 py-3 bg-rose-50 border border-rose-250 dark:bg-rose-950/20 dark:border-rose-800 rounded-xl text-rose-700 dark:text-rose-400">
          ...
          <button
            onClick={handleResolveAbnormality}
          ...
          {activeJob ? (
              ...
              {!isBtReadyForProduction && (
                <div className="absolute inset-0 z-30 bg-white/95 dark:bg-slate-950/95 backdrop-blur-md rounded-2xl p-6 sm:p-8 flex flex-col items-center justify-center text-center shadow-lg border border-amber-300 dark:border-amber-900 animate-in fade-in duration-200 select-none">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed max-w-md mb-6 animate-pulse">
                    Koneksi printer Bluetooth terputus atau tablet baru ter-reset. Untuk menjamin cetakan Kanban &
                    Label produksi valid, sambungkan Bluetooth Printer sebelum memproses job ini.
                  </p>
                  ...
                  <div className="p-6 flex flex-col items-center text-center">
                    <div className="w-full max-w-2xl mb-6 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl p-5 relative overflow-hidden text-left">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-slate-400 font-medium mt-1">
                            Customer:{' '}
                            <span className="font-bold text-slate-600 dark:text-slate-400">{activeJob.customer}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-5 border-t border-slate-100 dark:border-slate-800">
                        <div className="bg-white dark:bg-slate-950 p-3 rounded-lg border border-slate-200/60 dark:border-slate-700 shadow-sm">
                          ...
                          <div className="text-sm font-black font-mono text-slate-700 dark:text-slate-300 mt-1">
                            {activeJob.mold}
                          </div>
                        </div>
                        <div className="bg-white dark:bg-slate-950 p-3 rounded-lg border border-slate-200/60 dark:border-slate-700 shadow-sm">
                          ...
                          <div
                            className="text-xs font-black font-mono text-slate-700 dark:text-slate-300 mt-1.5 truncate"
                            title={activeJob.material}
                          >
                            {activeJob.material}
                          </div>
                        </div>
                    ...
                    <div className="w-full max-w-2xl flex flex-col gap-3">
                      {shiftStartStatus.isBeforeStart && (
                        <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-xl p-3.5 text-amber-800 dark:text-amber-400 text-xs font-bold flex items-center justify-between shadow-sm animate-in fade-in">
                          <div className="flex items-center gap-2.5">
                            ...
                            <span>
                              Waktu shift/dandori belum dimulai (Jadwal start:{' '}
                              <strong className="font-black text-amber-900 dark:text-amber-300 font-mono">
                                {shiftStartStatus.startTimeStr}
                              </strong>
                              ). Tombol Finish Preparation dikunci hingga waktu tiba.
                            </span>
                          </div>
                          {activePortal !== 'member' && (
                      ...
                      >
                        ...
                        {isFirstInShift ? 'Finish Shift Preparation' : 'Finish Dandori Setup'}
                      </button>
                      ...
                      <button
                        onClick={() =>
                          closeShiftProduction(
                            machineKey,
                            activeJob.shift === 'night' ? 'night' : 'day',
                            selectedDate
                          )
                        }
                  ...
                  <div className="p-6">
                    ...
                    <div className="mb-6 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-150 dark:border-slate-800/80">
                      ...
                      <div className="flex justify-end mt-3 text-[10px] text-slate-400 font-bold">
                        <span>
                          Cycle Time: {activeJob.ct}s &nbsp;|&nbsp; Cavity: {activeJob.spec ?? 24}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <button
                          ...
                          disabled={
                            isReadOnlyMode || isReportingAbnormal || isReportingNg || (ngState.isNg && !abnormality.isAbnormal)
                          }
                      ...
                      <button
                        ...
                        disabled={
                          isReadOnlyMode || isReportingNg || isReportingAbnormal || (abnormality.isAbnormal && !ngState.isNg)
                        }
                      ...
                      >
                        ...
                        {ngState.isNg
                          ? isReportingNg
                            ? 'Under Investigation'
                            : 'Resolve NG Issue'
                          : 'Report NG (Quality Issue Ongoing)'}
                      </button>

                      <button
                        onClick={() => handleOpenSignOff(activeJob)}
                        disabled={
                          isReadOnlyMode || abnormality.isAbnormal || ngState.isNg || isReportingAbnormal || isReportingNg
                        }
                      ...
                      >
                        <Play className="w-4 h-4 text-slate-500" /> Close {activeJob.shift?.toUpperCase()} Shift
                        Production
                      </button>
                    </div>
                  </div>
            ...
            <div className="bg-white dark:bg-slate-950 border-2 border-indigo-500/40 dark:border-indigo-500/30 rounded-2xl p-6 sm:p-8 flex flex-col items-center justify-center text-center shadow-lg relative overflow-hidden animate-in fade-in">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md leading-relaxed mb-6 font-medium">
                Part terakhir (<strong className="font-mono text-slate-700 dark:text-slate-300">{finalDandoriJob.model}</strong>)
                di shift {finalDandoriJob.shift?.toUpperCase()} telah selesai diproduksi. Lakukan pembersihan mold,
                merapikan area kerja, dan selesaikan Dandori Akhir Shift sebelum menekan tombol{' '}
                <strong className="text-slate-700 dark:text-slate-300">Close Shift Production</strong>.
              </p>

              <div className="w-full max-w-md flex flex-col gap-3">
                <button
                  onClick={() => handleCompleteFinalDandori(finalDandoriJob.id)}
                  disabled={isReadOnlyMode}
                ...
                <div className="space-y-1">
                  <button
                    onClick={() => {
                      closeShiftProduction(
                        machineKey,
                        finalDandoriJob.shift === 'night' ? 'night' : 'day',
                        selectedDate,
                        userInitials,
                        mName,
                        true
                  ...
                  >
                    <CheckCircle2 className="w-4 h-4 text-emerald-400" /> Close{' '}
                    {finalDandoriJob.shift?.toUpperCase()} Shift Production
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
            <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800">
              <h3 className="text-[11px] font-black uppercase tracking-widest text-slate-400">
                Up Next in Queue — {selectedDate}
              </h3>
            </div>
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {jobs.length === 0 ? (
                <p className="py-8 text-center text-[11px] text-slate-400 font-bold uppercase tracking-wider">
                ...
                jobs.map((job, idx) => (
                  ...
                  >
                    <span className="w-6 h-6 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[10px] font-black text-slate-500 shrink-0">
                      {idx + 1}
                    </span>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-slate-550 truncate">
                        {job.partName} &nbsp;|&nbsp; Qty: {job.actualQty}/{job.qtyLot} &nbsp;|&nbsp; {job.timeRange}
                      </p>
                    </div>
            ...
            <div className="flex-1 flex flex-col min-h-[300px]">
              ...
              {isReportingAbnormal && (
                <div className="p-4 bg-rose-50 dark:bg-rose-950/20 border-b border-rose-100 dark:border-rose-900/40 animate-in slide-in-from-top-2 shrink-0">
                  <div className="flex justify-between items-center mb-1">
                    <h4 className="font-bold text-rose-800 dark:text-rose-450 text-xs uppercase">
                      Catat Jenis Abnormal ke Log
                    </h4>
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className="block text-[9px] font-extrabold uppercase tracking-wider text-rose-600 dark:text-rose-500 mb-1">
                        Tipe Abnormal
                      </label>
                      <select
                        value={selectedAbnType}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        onClick={handleSaveAbnormalityRecord}
                      ...
                      <button
                        onClick={handleCancelAbnormality}
              ...
              {isReportingNg && (
                <div className="p-4 bg-amber-50 dark:bg-amber-950/20 border-b border-amber-100 dark:border-amber-900/40 animate-in slide-in-from-top-2 shrink-0">
                  <div className="flex justify-between items-center mb-1">
                    <h4 className="font-bold text-amber-800 dark:text-amber-455 text-xs uppercase">
                      Catat Jenis NG ke Log
                    </h4>
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className="block text-[9px] font-extrabold uppercase tracking-wider text-amber-600 dark:text-amber-500 mb-1">
                        Tipe NG
                      </label>
                      <select
                        value={selectedNgType}
                        onChange={(e) => setSelectedNgType(e.target.value)}
              ...
              <div className="divide-y divide-slate-50 dark:divide-slate-900 overflow-y-auto flex-1 max-h-[450px]">
                {logList.length === 0 ? (
                  ...
                  logList.map((entry) => {
                    const isAbnormalLog =
                      entry.type === 'abnormal' || (entry.message || '').toUpperCase().includes('[ABNORMAL');
                    const isNgLog = entry.type === 'ng' || (entry.message || '').toUpperCase().includes('[NG');
                    const logColor = isAbnormalLog
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="bg-[#E76114] p-4 flex justify-between items-center text-white">
              ...
              <button onClick={() => setShowSignOff(false)} className="cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    min={0}
                    value={signOffOkQty}
                    onChange={(e) => setSignOffOkQty(e.target.value)}
                    className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-bold bg-white dark:bg-slate-950 dark:text-white outline-none focus:border-[#E76114]"
                  />
                </div>
              </div>
              ...
              >
                {pinLoading ? (
                  ...
                  <>
                    <ShieldCheck className="w-4 h-4" /> Konfirmasi & Selesaikan Job
                  </>
                )}
              </button>
            </div>
