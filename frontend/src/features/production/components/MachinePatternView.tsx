...
import { Calendar, Clock, Box, GripVertical, CheckCircle, Settings2, Trash2, Layers, Gauge, Database, RotateCcw, AlertTriangle, Sparkles, Wrench } from 'lucide-react';
import { DragDropContext, Droppable, Draggable } from '@hello-pangea/dnd';
import type { DropResult } from '@hello-pangea/dnd';
import { useQuery } from '@tanstack/react-query';
import { useProduction, getUniqueMachineKey, getTodayDateString, getHeijunkaJobsForMachine } from '../context/ProductionContext';
...
function ShiftTimeline({ shift, jobs, avgJobs, dayOT, nightOT, showDailyAndAct = true, isAbnormalActive = false, isNgActive = false, logs = [], activeAbnormalStart = '', activeNgStart = '' }: ShiftTimelineProps) {
  ...
  logs.forEach((log, index) => {
    ...
    if (msg.includes('NG RESOLVED')) {
      ...
      if (!startTime) {
        // Fallback: Find the most recent 'NG STARTED' log before this resolved log
        // (logs array is sorted newest first, so we look at elements after the current index)
    ...
    if (msg.includes('Abnormal RESOLVED')) {
      ...
      if (!startTime) {
        // Fallback: Find the most recent 'Abnormal STARTED' log before this resolved log
        const olderLogs = logs.slice(index + 1);
...
export function MachinePatternView({ machine, factory, machineKey: propsMachineKey, selectedDate }: MachinePatternViewProps) {
  ...
  const uniquePartsCount = React.useMemo(() => {
    ...
    const machineParts = parts.filter((p: any) => p.homeLine && p.homeLine.trim().toUpperCase().includes(machine.toUpperCase()));
           ...
           <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
             ...
             {canEditPattern ? (
               <div className="flex flex-wrap items-center gap-2 shrink-0">
                 ...
                 {/* OT Selectors for Day / Night Shift */}
                ...
                {hasProductionData && (
                  <div className="flex items-start gap-2 bg-red-100 border border-red-400 rounded-lg px-3 py-2">
                    ...
                    <div className="text-xs font-black text-red-800 leading-snug">
                      ...
                      <span className="font-bold text-red-650 block mt-0.5">Hanya reset di awal shift sebelum produksi mulai!</span>
                    </div>
                  </div>
                )}
                    ...
                    <div>
                      ...
                      {adjustmentScope !== 'monthly' && (
                        <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">
                          Pilih "Template Bulanan" untuk menyamakan dengan rata-rata bulanan, atau "Master & Forecast" untuk menghitung ulang dari database parts & forecast terbaru.
                        </p>
                      )}
                    </div>
                  </div>
         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 w-full">
           ...
           <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center gap-3 transition-all hover:shadow-md hover:border-slate-300 select-none">
             ...
             <div>
               ...
               <div className="text-[9px] text-slate-500 font-extrabold leading-none mt-0.5">{totalShots.toLocaleString()} Shots</div>
             </div>
           </div>
           ...
           <DragDropContext onDragEnd={handleDragEnd}>
             <section>
               <div className="flex items-center gap-4 mb-4 select-none">
                 <h3 className="text-[10px] font-black text-slate-700 uppercase tracking-widest bg-slate-100 border border-slate-200 px-4 py-1.5 rounded-full shadow-sm">Job Lineup</h3>
                 <div className="h-px bg-slate-200 flex-1"></div>
                 {canEditPattern && (
                   <div className="flex gap-2 items-center shrink-0">
                     <span className="text-[8px] bg-indigo-50 border border-indigo-250 text-indigo-700 px-3 py-1 rounded-full font-black uppercase tracking-wider shadow-sm">Supervisor Mode</span>
                   </div>
                 )}
               </div>
